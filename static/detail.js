...
(function () {
  ...
  function renderDetail(r) {
    ...
    const also = r.entity_id
      ? `<a href="/entidad?id=${encodeURIComponent(r.entity_id)}" class="rh-action mt-4 inline-flex items-center gap-1.5 bg-ink-900 px-3 py-2 text-sm text-white hover:bg-ink-600">Ver en todas las fuentes →</a>` : "";
    ...
    if (drawerBody) drawerBody.innerHTML = `
      ${r.image_url ? `<img src="${escapeHtml(r.image_url)}" alt="Foto de ${escapeHtml(r.person_name || r.title || "registro")}" referrerpolicy="no-referrer" class="mb-4 max-h-56 w-full object-cover ring-1 ring-slate-200" onerror="this.remove()">` : ""}
